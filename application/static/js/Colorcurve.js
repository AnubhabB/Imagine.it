...
ColorCurve.prototype.mouseDown = function(event) {
	...
	console.log(this.c.offsetLeft+" "+this.c.offsetTop);
	var correctY = $(".CurvesPreview").offset().left;
    var correctX = $(".CurvesPreview").offset().top;
    var x = (event.pageX-this.c.offsetLeft-correctY)/this.width,
        y = (event.pageY-this.c.offsetTop-correctX)/this.height;
        console.log("X "+x+" Y "+y);
...
ColorCurve.prototype.mouseMove = function(event) {
   ...
   'use strict';
	...
	var correctY = $(".CurvesPreview").offset().left;
    var correctX = $(".CurvesPreview").offset().top;
    //alert(correctX+" "+correctY);
	var x = (event.pageX-this.c.offsetLeft-correctY)/this.width;
    var y = 1.0-((event.pageY-this.c.offsetTop-correctX)/this.height);
...
Filter.Init = function(img,contxt) 
{
...
*/
  ...
  this.canvas = document.getElementById('temp_canvas');
...
Filter.applyFilter = function() {
  ...
  for (var i = 0; i < Filter.numPixels; i++) {  
	pix[i*4]   = CC1.rgb[Filter.originalPixels[i*4]];  //Red
	pix[i*4+1] = CC2.rgb[Filter.originalPixels[i*4+1]];  //Green
	pix[i*4+2] = CC3.rgb[Filter.originalPixels[i*4+2]];  //blue
